      ...
      <div className="border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="h-16 lg:h-20 flex items-center justify-between">
            ...
            <Link href="/">
              <a className="absolute left-1/2 -translate-x-1/2 top-0 lg:-top-7 z-20 flex items-center justify-center w-24 h-24 lg:w-32 lg:h-32">
                <img src="/images/logo-dulce-bebe.svg" className="w-full h-full object-contain drop-shadow-[0_10px_18px_rgba(15,23,42,0.18)]" alt="Dulce Bebé" />
              </a>
            </Link>
            <div className="flex items-center gap-2 lg:gap-6 ml-auto">
              <div className="hidden lg:block relative">
                ...
                {categoriesVisible && <div className="absolute right-0 mt-1 w-64 bg-white border border-gray-200 rounded-2xl shadow-xl p-2"><div className="max-h-72 overflow-y-auto no-scrollbar">{categories?.map((category) => <Link key={category.id} href={`/accessories/${category.id}`} passHref><a onClick={() => setCategoriesVisible(false)} className="block px-4 py-2.5 rounded-xl text-sm text-gray-700 hover:bg-palette-slighter hover:text-palette-dark">{category.name}</a></Link>)}</div></div>}
              </div>
              <Link href="/about/inicio"><a className="hidden lg:block py-3 text-sm font-bold text-gray-800 hover:text-palette-sdark">Quiénes somos</a></Link>
              <div className="hidden sm:flex items-center justify-center w-9 h-9 rounded-full border border-gray-200 bg-white shadow-sm"><UserSession session={session} /></div>
          ...
          <nav className={`${isShow ? "block" : "hidden"} lg:hidden border-t border-gray-100 pb-3`}>
            ...
            {categoriesVisible && <div className="pl-3 pb-2">{categories?.map((category) => <Link key={category.id} href={`/accessories/${category.id}`} passHref><a className="block py-2 text-sm text-gray-600">{category.name}</a></Link>)}</div>}
            <Link href="/about/inicio"><a className="block py-3 text-sm font-bold text-gray-700">Quiénes somos</a></Link>
            {session?.user?.role?.includes("ADMIN") && <Link href="/admin"><a className="block py-3 text-sm font-bold text-gray-700">Administración</a></Link>}
          </nav>
        </div>
      </div>
    </header>
