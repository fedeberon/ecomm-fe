...
function FilterModal({ filterParams, searchFunction, columnList }) {
    ...
    const handleChangeSubCat = (e, index) => {
        ...
        updatedQueryParameters[index] = updatedQueryParameters[index] || [];
        updatedQueryParameters[index].push(e.target.value);
    ...
    //Changes the value of the term that'll be searched. Is invoked by a useEffect() to update it with a small delay.   
    ...
    function searchButton() {
        setShowFilter(false);
        searchFunction([searchTerm, queryParameters, selectedOrderCol, ascOrder]);
    }

    //Debounces the search reducing the updating time for the   
    useEffect(() => {
        const debouncedSearch = debounce(searchFunction, 500); //500 ms delay
