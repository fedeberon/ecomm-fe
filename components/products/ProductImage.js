...
function ProductImage({ images = [], id }) {
  ...
  return (
    <div className="w-full max-w-[520px] mx-auto">
      <div className="relative overflow-hidden rounded-[20px] bg-white border border-slate-100 h-[360px] sm:h-[410px] lg:h-[440px] flex items-center justify-center">
        <img src={selectedImage?.link?.src || selectedImage?.link} alt={selectedImage?.url || 'Imagen del producto'} className="w-full h-full object-contain p-4 sm:p-5 transition duration-300" />
        {safeImages.length > 1 && <>
          <button type="button" onClick={previousImage} aria-label="Imagen anterior" className="absolute left-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-white/95 border border-slate-200 shadow flex items-center justify-center text-slate-600 hover:text-palette-sdark"><FontAwesomeIcon icon={faArrowLeft} className="w-3" /></button>
          <button type="button" onClick={nextImage} aria-label="Imagen siguiente" className="absolute right-3 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-white/95 border border-slate-200 shadow flex items-center justify-center text-slate-600 hover:text-palette-sdark"><FontAwesomeIcon icon={faArrowRight} className="w-3" /></button>
        </>}
      </div>

      {safeImages.length > 1 && <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
        {safeImages.map((imgItem, index) => <div key={`${imgItem.url}-${index}`} className="relative flex-shrink-0">
          <button type="button" onClick={() => setSelectedIndex(index)} className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-white border-2 overflow-hidden transition ${selectedIndex === index ? 'border-palette-sdark shadow-sm' : 'border-slate-100 hover:border-slate-200'}`}>
      ...
      {safeImages.length > 1 && <p className="mt-1 text-[11px] text-slate-400">{selectedIndex + 1} de {safeImages.length} imágenes</p>}
